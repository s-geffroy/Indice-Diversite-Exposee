...
in ten remain undecidable. What a regulator must require is therefore not the rank but the
\textbf{verifiable} rank. We
therefore specify, under Article~40 of the \emph{Digital Services Act}, aggregate tables
...
\S\ref{sec:pageeffect} demotes that column to a means of verification.
...
\textbf{portable} again, which was the real stake: were it page-dependent, no regulatory floor
could be written without describing every page served. The \texttt{format} column of
\S\ref{sec:request} is still requested --- it allows this check to be reproduced elsewhere --- but
it no longer conditions the computation. What a regulator must require first is the displays
...
\paragraph{Consequence for \S\ref{sec:request}.} The request asked for ``the served rank''. That
is not enough: a platform can supply an order column that is not the rank, without lying and with
nothing signalling it. It must therefore ask for a \textbf{verifiable} rank, and the
exchangeability test becomes an admissibility clause applied to the delivered data, before any
other measurement.
...
\subsection{What privacy costs an audit}
\label{sec:privacy}

\S\ref{sec:request} proves four aggregate tables suffice; it says nothing about their
harmlessness. An aggregate is not anonymous, and the small-cell suppression threshold used so far
offers no formal guarantee. We therefore publish the same quantities under differential privacy,
and measure the price.

\paragraph{Two costs, and the second dominates.} Laplace \textbf{noise} protects against
inference about one row, and its effect depends on the \emph{density} of the published table: on
the nine cells of the rank margin it stays invisible down to $\varepsilon = 0.03$; on the
$451{,}100$ (item, rank) cells, $99.7\%$ of which carry fewer than five impressions, it destroys
everything. The \textbf{contribution bound} is what makes a \emph{per-person} guarantee
possible, and it is what shifts the figures.

\paragraph{The regulated quantity holds.} The share of the population below the floor does not
move by a thousandth between $\varepsilon = 10$ and $\varepsilon = 0.1$. Bounding at one day per
reader, however, adds $2.8$ points to a share of $13.7\%$, and at five days still $0.7$ point ---
always in the same direction. Assiduous readers are more diverse than the rest: dropping them
makes the platform look \emph{less} compliant than it is. The error committed in protecting
readers therefore does not exonerate the platform.

\paragraph{Severity does not hold.} At $\varepsilon = 1$, the usual reference value, $\eta$ goes
from $0.882 \pm 0.046$ to $0.505 \pm 0.051$. Noise on counts acts as measurement error: it
attenuates towards zero, and the standard error does not say so. The consequence is directional
--- an understated severity makes attention look flatter, hence burial milder. The publication
threshold corrects the level then overshoots it, trading attenuation for \textbf{selection}:
$0.881$ over $57$ items at $\varepsilon = 3$, but $1.03$ over $25$ at $\varepsilon = 1$ and
$1.20$ over nine at $0.3$.

\paragraph{The knot.} The rank margin survives any budget and lands within $0.7\%$ of the
item-fixed-effects value. But we only know that because we hold the item-level table, the one no
guarantee protects: \emph{what publishes safely is what cannot be checked}. No privacy budget
reconciles the two.

\paragraph{Consequence for \S\ref{sec:request}.} The cell table must not be requested in this
form. Three requests replace it: the rank margin of displays, publishable at $\varepsilon = 0.1$;
the item-fixed-effects estimate \emph{computed by the platform} and published as a noised scalar,
whose sensitivity is bounded; and the gap between the two, the only available check. The
contribution bound must be published with the result, as the suppression threshold is.

\paragraph{Reservations.} The mechanism used is the simplest there is: this chapter bounds the
cost from above, it does not claim to attain it. The per-user guarantee is measured only on
EB-NeRD, the only log carrying a reader identifier; on Baidu-ULTR only a per-impression guarantee
is possible, which is a property of the dataset, not of the method.
...
independence would give $25\%$. A regulator must therefore say \textbf{which axis} it regulates,
...
\subsection{What a floor costs, on real feeds}
\label{sec:floorprice}

\S\ref{sec:baselines} prices a diversity standard between $8.2$ and $18.9\%$ of engagement, but
assumes relevance known and bears on simulated feeds. The user-days of
\S\ref{sec:indexmeasured} allow the question to be asked without assuming anything: how many
served items would have to be \textbf{replaced}?

\paragraph{The check first.} The cost is computed greedily --- each substitution removes an item
from the most-served section and adds one to the least-served. Confronted with exhaustive
enumeration of every redistribution over two hundred short compositions, it coincides $200$ times
out of $200$. The price that follows is therefore not a cautious bound.

\paragraph{The proposed floor constrains almost nothing.} Lifting the whole population above
$0.40$ requires replacing \textbf{one served item in $177$} --- $0.57\%$ of the log --- and the
median user-day needing treatment requires a \emph{single} substitution. The price is convex:
$0.21\%$ at a floor of $0.30$, $2.27\%$ at $0.50$, $8.24\%$ at $0.60$, $17.11\%$ at $0.70$. The
level used so far sits at the painless end of the curve.

\paragraph{And the shape of the regulated quantity is at fault.} \S\ref{sec:index} prescribes the
\emph{share of the population below the threshold}, precisely so a satisfactory mean cannot hide
an enclosed minority. But a share is a threshold, and a threshold is met at the margin: the
platform fixes readers \emph{just} below and leaves the most enclosed where they are. The quantity
chosen against masking by the mean invites treating the margin.

\paragraph{A ceiling nothing lifts.} An entropy cannot exceed the logarithm of the number of items
\emph{served}: a reader receiving five caps out at $0.49$. At a floor of $0.60$, $10.9\%$ of
user-days are therefore out of reach whatever the platform does; at $0.80$, $22.9\%$. These are
not enclosed readers but \emph{light} ones, and the only possible compliance is to serve them more
--- which a diversity standard has no reason to encourage. Two ways out, and one must be chosen:
exempt short sessions, or normalise the index by the attainable maximum rather than by
$\log_2 k$.

\paragraph{Reservations.} The cost is measured in items exchanged, not in engagement: EB-NeRD
provides no relevance model. The computation further assumes any section can be served to anyone,
which overstates ease.
