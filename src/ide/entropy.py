...
def attainable_index(served: int, catalogue_size: int) -> float:
    """Indice maximal qu'un fil de ``served`` contenus peut atteindre.

    Un plancher réglementaire porte sur une entropie normalisée, et une entropie ne peut pas
    dépasser le logarithme du nombre de **contenus servis**. Un lecteur qui n'en reçoit que
    cinq ne peut donc pas franchir un plancher élevé, quoi que fasse la plateforme.

    .. danger::
        Ce plafond n'est pas une limite de mesure, c'est une limite de l'**objet régulé**. Il
        signifie qu'un plancher trop haut régule le **volume** de consommation sous couvert de
        réguler la diversité : la seule façon d'y conformer un lecteur léger est de lui en
        servir davantage.

    Examples:
        >>> round(attainable_index(5, catalogue_size=26), 3)
        0.494
        >>> attainable_index(40, catalogue_size=26)
        1.0
    """
    if served < 1:
